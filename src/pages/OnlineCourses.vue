  ...
  <div class="min-h-screen bg-[#0F1C2D]">
    <div class="max-w-6xl mx-auto px-4 py-8">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div class="flex gap-2">
          <button 
            ...
            :class="['px-5 py-2.5 rounded-xl transition-all font-medium', activeTab === tab.id ? 'bg-[#4A5F8B] text-white shadow-lg shadow-[#4A5F8B]/30' : 'bg-[#1E2532] text-[#6B7C93] hover:text-white hover:bg-[#2D3748]']"
          >
            {{ tab.name }}
          </button>
        </div>
      ...
      <div class="flex flex-col sm:flex-row gap-4 mb-8">
        ...
        <select 
          v-model="sortBy"
          class="px-4 py-3 bg-[#1E2532] border border-[#4A5F8B] text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4A5F8B] transition-all cursor-pointer"
        >
        ...
        >
          <div class="relative">
            ...
            <div class="absolute top-4 left-4 flex gap-2">
              <span class="px-3 py-1 bg-[#4A5F8B] text-white text-xs rounded-full font-medium">{{ course.category }}</span>
              <span v-if="course.isNew" class="px-3 py-1 bg-pink-500 text-white text-xs rounded-full font-medium">新课</span>
            ...
            <div class="absolute bottom-4 right-4 bg-black/60 px-3 py-1 rounded-full text-white text-sm">
              <i class="fa-regular fa-clock mr-1"></i>{{ course.duration }}
            </div>
          </div>
          <div class="p-6">
            <div class="flex items-center gap-1 mb-3">
              <div class="flex">
                <i v-for="i in 5" :key="i" :class="['fa-solid fa-star text-sm', i <= course.rating ? 'text-yellow-400' : 'text-[#6B7C93]']"></i>
              </div>
            ...
            <div class="flex items-center gap-4 mb-4 text-sm text-[#6B7C93]">
              ...
              <span><i class="fa-solid fa-users mr-1"></i>{{ course.studentCount }}学员</span>
            </div>
            <div class="flex items-center justify-between pt-4 border-t border-[#4A5F8B]/20">
              <div class="flex items-center gap-3">
                <img :src="course.instructor.avatar" :alt="course.instructor.name" class="w-10 h-10 rounded-full border-2 border-[#4A5F8B]" />
                <div>
                  <p class="text-sm text-white font-medium">{{ course.instructor.name }}</p>
                  <p class="text-xs text-[#6B7C93]">{{ course.instructor.title }}</p>
                </div>
              </div>
              <div class="text-right">
                <span v-if="course.isFree" class="text-xl font-bold text-green-400">免费</span>
                <div v-else>
                  <span v-if="course.originalPrice" class="text-sm text-[#6B7C93] line-through mr-2">¥{{ course.originalPrice }}</span>
                  <span class="text-xl font-bold text-[#4A5F8B]">¥{{ course.price }}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-if="filteredCourses.length === 0" class="text-center py-16">
        <div class="w-20 h-20 bg-[#1E2532] rounded-full flex items-center justify-center text-[#4A5F8B] mx-auto mb-4">
          <i class="fa-solid fa-search text-3xl"></i>
        </div>
        <h3 class="text-xl font-semibold text-white mb-2">未找到相关课程</h3>
        <p class="text-[#B8C6D8]">请尝试调整搜索条件或筛选条件</p>
      </div>

      <div class="mt-12 flex justify-center">
        <nav class="flex items-center gap-2 bg-[#1E2532] p-2 rounded-xl border border-[#4A5F8B]">
          <button class="px-4 py-2 rounded-lg text-[#6B7C93] hover:bg-[#4A5F8B] hover:text-white transition-colors">
            <i class="fa-solid fa-chevron-left"></i>
          </button>
          <button class="px-4 py-2 rounded-lg bg-[#4A5F8B] text-white font-medium">1</button>
          <button class="px-4 py-2 rounded-lg text-[#6B7C93] hover:bg-[#4A5F8B] hover:text-white transition-colors">2</button>
          <button class="px-4 py-2 rounded-lg text-[#6B7C93] hover:bg-[#4A5F8B] hover:text-white transition-colors">3</button>
          <span class="px-2 text-[#6B7C93]">...</span>
          <button class="px-4 py-2 rounded-lg text-[#6B7C93] hover:bg-[#4A5F8B] hover:text-white transition-colors">10</button>
          <button class="px-4 py-2 rounded-lg text-[#6B7C93] hover:bg-[#4A5F8B] hover:text-white transition-colors">
